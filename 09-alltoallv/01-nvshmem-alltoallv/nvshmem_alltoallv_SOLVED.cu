...
__global__ void nvshmem_alltoallv_kernel(
  ...
  // All CTAs have issued their chunks before the controller CTA quiets the
  // source side.  Cooperative launch keeps the later waits from starving an
  // unscheduled producer CTA.
  grid.sync();
  nvshmemx_qp_handle_t all_qps = NVSHMEMX_QP_ALL;
  if (blockIdx.x == 0) {
    if (quiet_scope == QpQuietScope::block) {
      nvshmemx_qp_quiet_block(NVSHMEMX_PE_ALL, &all_qps, 1);
    } else if (quiet_scope == QpQuietScope::warp) {
      if (threadIdx.x < warpSize)
        nvshmemx_qp_quiet_warp(NVSHMEMX_PE_ALL, &all_qps, 1);
