...
__device__ void send_and_deliver_local(ncclDevComm dev_comm, ncclGin gin,
  ...
  for (int step = 0; step < lsa.nRanks; ++step) {
    ...
    value_type *target = static_cast<value_type *>(ncclGetLsaPointer(
        ...
        destination_local));
    copy_values(source, target, count);
  }
  ...
  if (threadIdx.x == 0) {
    ...
    for (int task = 0; task < task_count; ++task) {
      ...
      for (int shard = 0; shard < route_shards; ++shard) {
        ...
        if (count == 0)
          continue;
        gin.put(rail, destination_domain, inbox_window,
...
__device__ void wait_and_scatter(ncclDevComm dev_comm, ncclGin gin,
  ...
  __syncthreads();
  if (expected != 0)
    gin.waitSignal(ncclCoopCta(), data_signal, epoch * expected);
  ...
  for (int task = 0; task < task_count; ++task) {
    ...
    for (int shard = 0; shard < route_shards; ++shard) {
      ...
      if (count == 0)
        continue;
      const value_type *staged =
          static_cast<const value_type *>(ncclGetLocalPointer(
              inbox_window, source_domain * domain_bytes +
                                destination_local * slot_bytes +
                                offset * sizeof(value_type)));
      value_type *destination = static_cast<value_type *>(ncclGetLsaPointer(
          recv_window, (entry.recv_offset + offset) * sizeof(value_type),
          destination_local));
      copy_values(staged, destination, count);
    }
  }
  stamp<kProfile>(trace, &BlockTrace::scatter_end);

  // One flush protects this rank's send window for reuse; one world barrier
  // publishes the collective. Neither runs per shard.
...
void require_cooperative_grid(const alltoallv::nccl_setup::State &state,
  ...
  if (state.rank == 0) {
    // The best CTA count for this kernel is usually near this limit, so print
    // it: it is the top of the range worth sweeping.
    std::printf("NCCL hybrid launch: %d CTAs of %d threads, cooperative grid "
                "limit %d\n",
                options.blocks, options.threads, collective_limit);
  }
...
void report_phase_profile(const alltoallv::Options &options,
                          const char *implementation, float elapsed_ms,
                          const double phase_ms[5]) {
  int rank = 0;
  alltoallv::mpi_check(MPI_Comm_rank(MPI_COMM_WORLD, &rank),
                       "MPI_Comm_rank(phase profile)");
  // Report one rank's phases, not a per-phase maximum: taking the maximum of
  // each phase independently mixes ranks and no longer partitions the elapsed
  // time. The slowest rank is the one that defines the collective's cost.
  struct {
    double value;
    int rank;
  } local{elapsed_ms, rank}, slowest{};
  alltoallv::mpi_check(MPI_Allreduce(&local, &slowest, 1, MPI_DOUBLE_INT,
                                     MPI_MAXLOC, MPI_COMM_WORLD),
                       "MPI_Allreduce(phase profile critical rank)");
  double critical[5];
  for (int phase = 0; phase < 5; ++phase)
    critical[phase] = phase_ms[phase];
  alltoallv::mpi_check(MPI_Bcast(critical, 5, MPI_DOUBLE, slowest.rank,
                                 MPI_COMM_WORLD),
                       "MPI_Bcast(phase profile)");
  if (rank != 0)
    return;
  const double total =
