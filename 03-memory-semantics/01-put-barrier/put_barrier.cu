...
__global__ void put_then_barrier(const int *source, int *payload, int *result,
                                 int rank) {
  if (rank == 0) {
    /* TODO: put source into payload on PE 1 with the typed device put. */
  }
  nvshmem_barrier_all();
  *result = *payload;
}
...
int main(int argc, char **argv) {
  ...
  if (source == NULL || payload == NULL || result == NULL)
    MPI_Abort(MPI_COMM_WORLD, 1);
  ...
  void *args[] = {&source, &payload, &result, &context.rank};
  launch_collectively(reinterpret_cast<const void *>(put_then_barrier), args,
                      context.stream);
