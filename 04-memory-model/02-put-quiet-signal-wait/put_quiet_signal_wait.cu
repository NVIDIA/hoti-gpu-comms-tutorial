...
__global__ void put_quiet_signal_wait(const int *source, int *payload,
                                      uint64_t *ready, int *result, int rank) {
  if (rank == 0) {
    /* TODO: put, quiet, then signal ready on PE 1 with device APIs. */
  } else {
...
int main(int argc, char **argv) {
  ...
  launch_collectively(reinterpret_cast<const void *>(put_quiet_signal_wait),
                      args, context.stream);
